...
OUT_ROOT=${OUT_ROOT:-"$WORK_ROOT/runs"}
...
if [ "$TRANSLATED_TXT_ROOT" = "$SOURCE_TXT_ROOT" ]; then
  echo "[MMGCN-UNIFIED] refusing to overwrite SOURCE_TXT_ROOT: $SOURCE_TXT_ROOT" >&2
  exit 1
fi

if [ "$TRANSLATE" = "1" ]; then
  echo "[MMGCN-UNIFIED] translate eJSL txt to English"
...
if [ "$REBUILD_FEATURES" = "1" ] || [ ! -f "$MELD_UNIFIED_PKL" ] || [ ! -f "$EJSL_UNIFIED_PKL" ]; then
  echo "[MMGCN-UNIFIED] build same-origin MELD/eJSL feature pkl"
...
echo "[MMGCN-UNIFIED] reports: $OUT_ROOT"
