...
TELME_SAVE_ROOT=${TELME_SAVE_ROOT:-./MELD/save_model_meld_modalities}
OUT_ROOT=${OUT_ROOT:-./IEMOCAP/outputs_ejsl_telme_meld_modalities}
...
for csv_path in \
  ...
  if [ ! -f "$csv_path" ]; then
    echo "[TELME] missing MELD csv: $csv_path" >&2
    echo "[TELME] set MELD_RAW_ROOT to the unpacked MELD.Raw directory." >&2
    exit 1
  fi
done
...
echo "[TELME] train MELD modalities: $MODALITIES"
CUDA_VISIBLE_DEVICES="$GPU" python MELD/train_telme_meld_modalities.py \
  ...
  "${TRAIN_FLAGS[@]}" | tee "$OUT_ROOT/telme_meld_train.log"
...
for MOD in $MODALITIES; do
  case "$MOD" in
    ...
    *)
      echo "[TELME] unsupported modality in MODALITIES: $MOD" >&2
      exit 1
      ;;
  esac

  MODEL_ROOT="$TELME_SAVE_ROOT/$EVAL_MOD"
  PREFIX="telme_meld_${EVAL_MOD}_ejsl"
  LOG_PATH="$OUT_ROOT/${PREFIX}.log"

  echo "[TELME][$EVAL_MOD] test eJSL"
...
for MOD in $MODALITIES; do
  ...
  REPORT_PATH="$OUT_ROOT/telme_meld_${EVAL_MOD}_ejsl_classification_report.txt"
  echo "===== ${EVAL_MOD} ====="
