...
WORK_ROOT="${WORK_ROOT:-/raid_zoe/home/lr/maokeyu/sign/ecerc_bobsl_meld_ejsl}"
...
MELD_PKL="${MELD_PKL:-$FEATURE_ROOT/meld_ecerc_video_anjs4.pkl}"
EJSL_PKL="${EJSL_PKL:-$FEATURE_ROOT/ejsl_ecerc_video_anjs4_translated.pkl}"
BOBSL_TRAIN_VAL_PKL="${BOBSL_TRAIN_VAL_PKL:-$FEATURE_ROOT/bobsl_ecerc_video_anjs4_train_val.pkl}"
BOBSL_TEST_PKL="${BOBSL_TEST_PKL:-$FEATURE_ROOT/bobsl_ecerc_video_anjs4_test.pkl}"
...
export EJSL_PKL EPOCHS BATCH_SIZE LR L2 LOSS_GAMMA BASE_LAYER HIDDEN_SIZE CLASS_WEIGHT VIDEO_MODEL VIDEO_PROCESSOR
...
build_features_if_needed() {
  ...
  if [[ "${#skip_args[@]}" -eq 3 ]]; then
    echo "[ECERC-RUN] feature pkls already exist under $FEATURE_ROOT"
    return
  fi

  run_logged "$LOG_ROOT/build_features.log" \
    python new/ECERC/MELD/build_ecerc_video_anjs_features.py \
...
joint_pkl_for_config() {
  ...
  if [[ "$REBUILD_JOINT" == "1" || ! -s "$out_pkl" ]]; then
    run_logged "$LOG_ROOT/build_${config}.log" \
      python new/ECERC/MELD/build_ecerc_joint_video_pkl.py \
        --meld_pkl "$MELD_PKL" \
...
data = {
    ...
    "loss_gamma": float(os.environ["LOSS_GAMMA"]),
    "base_layer": int(os.environ["BASE_LAYER"]),
...
run_trial() {
  ...
  if [[ "$RESUME_DONE" == "1" && -s "$done_path" ]]; then
    echo "[ECERC-RUN] skip existing $out_dir"
    return
  fi
  ...
  run_logged "$LOG_ROOT/${config}_seed${seed}.log" \
    python new/ECERC/MELD/train_eval_ecerc_video_anjs.py \
      --train_pkl "$train_pkl" \
...
for config in $CONFIGS; do
  case "$config" in
    ...
    joint_b10k)
      ...
      ;;
    *)
      echo "[ECERC-RUN] unknown config: $config" >&2
      exit 2
      ;;
  esac
...
echo "[ECERC-RUN] done. Summary: $OUT_ROOT/video_joint_group_averages.txt"
