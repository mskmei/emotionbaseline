...
if [ "x$FRAME_ROOT" = "x" ]; then FRAME_ROOT="/raid_zoe/home/lr/wangyi/sign/eJSL_dial/frame"; fi
...
if [ "${TRANSLATE_TO_EN}" = "1" ]; then
  python prepare_emotrans_dial_test.py --base_dataset "${BASE_DATASET}" --new_dataset "${NEW_DATASET}" --dial_test_csv "${DIAL_TEST_CSV}" --frame_root "${FRAME_ROOT}" --txt_root "${TXT_ROOT}" --translate_to_en --translation_model "${TRANSLATION_MODEL}"
else
  python prepare_emotrans_dial_test.py --base_dataset "${BASE_DATASET}" --new_dataset "${NEW_DATASET}" --dial_test_csv "${DIAL_TEST_CSV}" --frame_root "${FRAME_ROOT}" --txt_root "${TXT_ROOT}"
fi
